# post_handball.R  (Weg 1: Self-Mirror im eigenen Repo)
library(atrrr)
library(httr2)
library(xml2)

original_url <- "https://www.handball-world.news/feed.xml"
mirror_file  <- "feed/handball.xml"     # eigene Kopie im Repo
...
# ---- Feed beschaffen: erst frisch (+Kopie speichern), sonst gespeicherte Kopie ----
...
new_items <- new_items[rev(seq_len(nrow(new_items))), , drop = FALSE]  # aelteste zuerst
...
# ---- Posten (mit #handball, max. 300 Zeichen) ----
...
for (i in seq_len(nrow(new_items))) {
  ...
  if (nchar(title) > budget) {
    title <- if (budget > 3) paste0(substr(title, 1, budget - 3), "...") else ""
  }
